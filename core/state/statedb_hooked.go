...
func (s *hookedStateDB) SubBalance(addr common.Address, amount *uint256.Int) {
	prev := s.inner.GetBalance(addr)
	s.inner.SubBalance(addr, amount)

	if s.hooks.OnBalanceChange != nil && !amount.IsZero() {
		newBalance := new(uint256.Int).Sub(prev, amount)
		// FIXME: reason not available
		s.hooks.OnBalanceChange(addr, prev.ToBig(), newBalance.ToBig(), tracing.BalanceChangeUnspecified)
	}
}

func (s *hookedStateDB) AddBalance(addr common.Address, amount *uint256.Int) {
	prev := s.inner.GetBalance(addr)
	s.inner.AddBalance(addr, amount)
...
func (s *hookedStateDB) SetNonce(address common.Address, nonce uint64) {
	// FIXME: walkaround, as set nonce interface has changed.
	prev := s.inner.GetNonce(address)
	s.inner.SetNonce(address, nonce)

	if s.hooks.OnNonceChangeV2 != nil {
		// FIXME: reason is not available in this interface
		s.hooks.OnNonceChangeV2(address, prev, nonce, tracing.NonceChangeUnspecified)
...
func (s *hookedStateDB) SetState(address common.Address, key common.Hash, value common.Hash) {
	prev := s.inner.GetState(address, key)
	s.inner.SetState(address, key, value)
...
func (s *hookedStateDB) SelfDestruct(address common.Address) {
	...
	prev := s.inner.GetBalance(address)
...
func (s *hookedStateDB) Selfdestruct6780(address common.Address) {
	...
	after := s.inner.GetBalance(address)
	changed := !after.Eq(prev)
