...
func (sim *simulator) processBlock(ctx context.Context, block *simBlock, header, parent *types.Header, headers []*types.Header, timeout time.Duration) (*types.Block, []simCallResult, error) {
	...
	blockContext := core.NewEVMBlockContext(header, sim.newSimulatedChainContext(ctx, headers), nil, sim.chainConfig)
	...
	if err := block.StateOverrides.Apply(sim.state); err != nil {
		return nil, nil, err
	}
	var (
		...
		vmConfig = &vm.Config{
			...
			// FIXME: Need to translate from hooks to EVMLogger
			// Tracer: tracer.Hooks(),
			Tracer: nil,
		}
	)
	...
	// FIXME: Need to pass transaction context
	evm := vm.NewEVM(blockContext, core.NewEVMTxContext(nil), tracingStateDB, sim.chainConfig, *vmConfig)
	...
	if header.ParentBeaconRoot != nil {
		// FIXME: core.ProcessBeaconBlockRoot has changed
		core.ProcessBeaconBlockRoot(*header.ParentBeaconRoot, evm, nil)
	}
	...
	for i, call := range block.Calls {
		...
		// FIXME: call.ToMessage has changed.
		// msg := call.ToMessage(header.BaseFee, !sim.validate, true)
		result, err := applyMessageWithEVM(ctx, evm, nil, timeout, sim.gp)
		if err != nil {
	...
	// FIXME: Check if following is required.
	// var requests [][]byte
	...
	// }
	blockBody := &types.Body{Transactions: txes, Withdrawals: []*types.Withdrawal{}}
	// FIXME: chainHeadReader has changed interface.
	// chainHeadReader := &simChainHeadReader{ctx, sim.b}
	b, err := sim.b.Engine().FinalizeAndAssemble(nil, header, sim.state, blockBody.Transactions, blockBody.Uncles, receipts, []*types.Withdrawal{})
	if err != nil {
		return nil, nil, err
	}
...
func (sim *simulator) activePrecompiles(base *types.Header) map[common.Address]vm.PrecompiledContract {
	// var (
